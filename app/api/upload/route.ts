  ...
  try {
    ...
    if (s3) {
      try {
        ...
        // Credenciais não disponíveis (dev local sem AWS configurado) — usa filesystem
        console.warn('[upload] S3 indisponível, usando filesystem:', s3Err instanceof Error ? s3Err.message : s3Err)
      }
    }
    ...
    return NextResponse.json(
      { error: process.env.NODE_ENV === 'development' ? `[dev] ${msg}` : 'Erro interno ao processar o upload' },
      { status: 500 }
    )
  }
}
