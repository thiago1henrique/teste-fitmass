...
// Used server-side — no raw card data
...
// Used client-side — extends base with card fields + conditional validation
export const checkoutSchema = baseCheckoutSchema
  ...
  .superRefine((data, ctx) => {
    ...
    if (!data.cardExpiry || !/^\d{2}\/\d{2}$/.test(data.cardExpiry)) {
      ...
      const [mm] = data.cardExpiry.split('/')
      if (parseInt(mm, 10) < 1 || parseInt(mm, 10) > 12) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: 'Mês inválido — use o formato MM/AA (ex: 05/27)',
          path: ['cardExpiry'],
        })
      }
    }
...
export type BaseCheckoutFormData = z.infer<typeof baseCheckoutSchema>
