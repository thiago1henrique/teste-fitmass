...
const NEUTRAL = [51, 51, 51] as const   // #333333
const GREEN = [136, 189, 35] as const   // #88BD23
const BLUE = [37, 182, 235] as const    // #25B6EB
const MAX_TINT = 0.18
...
function SliderHandle({
  ...
  useEffect(() => {
    // Two gentle left-right cycles: 50→60→40→60→40→50
    const DURATION = 3200
    const AMPLITUDE = 10
    ...
    const delay = setTimeout(() => {
      ...
      const animate = (ts: number) => {
        ...
        let pos: number
        if (p < 0.2) {
          pos = 50 + AMPLITUDE * easeInOut(p / 0.2)
        } else if (p < 0.4) {
          pos = 50 + AMPLITUDE - 2 * AMPLITUDE * easeInOut((p - 0.2) / 0.2)
        } else if (p < 0.6) {
          pos = 50 - AMPLITUDE + 2 * AMPLITUDE * easeInOut((p - 0.4) / 0.2)
        } else if (p < 0.8) {
          pos = 50 + AMPLITUDE - 2 * AMPLITUDE * easeInOut((p - 0.6) / 0.2)
        } else {
          pos = 50 - AMPLITUDE + AMPLITUDE * easeInOut((p - 0.8) / 0.2)
        }
